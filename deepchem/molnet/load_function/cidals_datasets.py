...
cidals_URL ="https://drive.google.com/file/d/112sjOO9DS9jisofBk-M2wjfEA-udcdUS/view?usp=sharing"
...
def load_cidals(featurizer='ECFP',
  ...
  """
  logger.info("About to load cidals dataset.")
  if data_dir is None:
  ...
  if reload:
    save_folder = os.path.join(save_dir, "cidals-featurized", str(featurizer))
  ...
  cidals_tasks = dataset.columns.values[1:].tolist()

  if reload:
    loaded, all_dataset, transformers = deepchem.utils.save.load_dataset_from_disk(
        save_folder)
    if loaded:
      return cidals_tasks, all_dataset, transformers

  # Featurize cidals dataset
  logger.info("About to featurize cidals dataset.")
  if featurizer == 'ECFP':
  ...
  logger.info("cidals tasks: %s" % str(cidals_tasks))
  logger.info("%d tasks in total" % len(cidals_tasks))

  loader = deepchem.data.CSVLoader(
      tasks=cidals_tasks, smiles_field="smiles", featurizer=featurizer)
  dataset = loader.featurize(dataset_file)
  logger.info("%d datapoints in cidals dataset" % len(dataset))
  ...
  if split == None:
    return cidals_tasks, (dataset, None, None), transformers
